    ...
    def build_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
        ...
        for v in model.modules():
            if hasattr(v, 'bias') and isinstance(v.bias, nn.Parameter):  # bias (no decay)
                g[2].append(v.bias)
            if isinstance(v, bn):  # weights (no decay)
                g[1].append(v.weight)
            elif hasattr(v, 'weights') and isinstance(v.weight, nn.Parameter):  # weights (with decay)
                g[0].append(v.weight)
        ...
        LOGGER.info(f"{colorstr('optimizer:')} {type(optimizer).__name__}(lr={lr}) with parameter groups "
                    f'{len(g[1])} weights(decay=0.0), {len(g[0])} weights(decay={decay}), {len(g[2])} bias')
        return optimizer
